...
decoding via almost linear-time algorithms~\cite{higgott2022pymatching}
...
errors cannot trigger any detectors while flipping a logical observable,
...
CryptoMiniSat~\cite{soos2016cryptominisat}.
...
\Cref{tab:z3_results} shows the results using Z3~\cite{de2008z3}. Z3
is the worst among
...
MaxSAT(RC2)~\cite{ignatiev2019rc2}. RC2 is
on par with CryptoMiniSat but has the advantage of not requiring any
prior knowledge and can find the exact error-correcting capability in one shot.
\begin{table}[h]
  \centering
  \caption{Results using MaxSAT}
